  ...
  useEffect(() => {
    if (jsonData && selectedVersion) {
      ...
      if (block && typeof block === 'object') {
        ...
        fields.forEach((f) => {
          if (block[f]) {
            html += `<div class="field-title" style="font-weight: 600; margin: 0.7em 0 0.3em;">${f}</div><div>${block[f]}</div>`;
          }
        });
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.examples}>
        <h4>사용 예시:</h4>
        <div>
          • 인라인 수식: <code>$E = mc^2$</code> 또는 <code>\(E = mc^2\)</code>
          <br />
          • 블록 수식: <code>$$\int_&#123;-\infty&#125;^&#123;\infty&#125; e^&#123;-x^2&#125; dx = \sqrt&#123;\pi&#125;$$</code>
          <br />
